...
router.get("/:id", function (req, res, next) {
  ...
  try {
    config = require(appRoot + `/config/${collection}_config.js`);
  } catch (err) {
    // This catch block would sanitize the collection name, if the config file of the given input name is not present in our server then this would catch block would trigger
  ...
  if (!(collectionItemId || collection || config)) {
    return res.status(400).json({
      message: "Invalid Collection",
    });
  }
  ...
  collectionItemTraits.forEach((collectionItemTrait) => {
    collectionItemTraitData[collectionItemTrait.trait_type_id] =
      collectionItemTrait.value;

    if (!ignoreTraits.includes(collectionItemTrait.trait_type.toLowerCase())) {
      ignoredCollectionItemTraitData[collectionItemTrait.trait_type_id] =
        collectionItemTrait.value;
    }
  });
  ...
  allTraitCountTypes.forEach((traitCount) => {
    allTraitCountTypesData[traitCount.trait_count] =
      traitCount.collectionItem_count;
  });
